...
export default function FAQsPage() {
  ...
  useEffect(() => {
    if (!router.isReady || router.asPath !== '/faqs#contact') return;
    ...
    window.setTimeout(scrollWithOffset, 100);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (ok && status === 200) {
        ...
        setSnack({
          open: true,
          msg: 'Your query is sent. Our support team will reach out to resolve your query.',
          severity: 'success',
        });
      } else {
      ...
      <Container maxWidth="md" className={classes.container}>
        <Box className={classes.categoryList}>
          {Object.keys(filteredFAQs).map((category) => (
            <FAQCategory
              key={category}
              title={filteredFAQs[category].title}
              items={filteredFAQs[category].items}
            />
          ))}
        </Box>
        ...
        <div ref={contactRef} id="contact" className={classes.contactHeader}>
          <Typography
            variant="h3"
            style={{ fontWeight: 700, color: '#1E293B', marginBottom: 8, textAlign: 'center' }}
          >
            Contact Us
          </Typography>
          <Typography
            style={{ fontSize: '1rem', color: '#64748B', textAlign: 'center', maxWidth: 600, margin: '0 auto' }}
          >
