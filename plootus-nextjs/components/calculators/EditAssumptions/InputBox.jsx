// =============================================================================
// InputBox — Next.js port
// Exact parity with packages/web/src/Utils/InputBox/InputBox.jsx
// =============================================================================
...
const InputBox = ({ value, handleChange }) => {
  ...
  const handleInputChange = (e) => {
    ...
    let stripped = inputVal.replace(/\$|,/g, '');
    let j = 0;
    for (let i = 0; i < stripped.length; i += 1) {
      if (stripped[i] === '0') { j += 1; } else { break; }
    }
    stripped = stripped.slice(j);

    if (isKhaliCheck(stripped)) { setVal('0'); }
    if (/^\d+$/.test(stripped)) { setVal(`${formatCommaString(stripped)}`); }
